document.addEventListener('DOMContentLoaded', function () {
    ...
    const hoy = new Date();
    const primerDiaMesAnterior = new Date(hoy.getFullYear(), hoy.getMonth() - 1, 1); // obtener mes anterior
    const ultimoDiaMesActual = new Date(hoy.getFullYear(), hoy.getMonth() + 1, 0); // obtener ultimo di del mes
    // asigna valor a las fechas
    ...
    function mostrarOrdenes(ordenes, aplicarFiltroFechas = true) {
        ...
        ordenes
            ...
            .sort((a, b) => {
                // Convertir las fechas de turno a objetos Date para comparar
    ...
    function aplicarFiltros(tipoFiltro, valorFiltro) {
        ...
        filas.forEach(fila => {
            const textoFila = fila.querySelector(`td:nth-child(${tipoFiltro === 'dominio' ? 4 : tipoFiltro === 'cliente' ? 3 : 9})`).textContent.toLowerCase(); // obteer los valores
            if (valorFiltro === '' || textoFila.includes(valorFiltro.toLowerCase())) {
                fila.style.display = '';
                sumaTotal += parseFloat(fila.querySelector('td:nth-child(7)').textContent);  // extrae el valor numérico de la celda que contiene el monto total
            } else {
